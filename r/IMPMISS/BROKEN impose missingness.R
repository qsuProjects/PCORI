...
impose_missingness = function( data, outcome.name, start.time.name, stop.time.name, 
  ...
  ##### Step 3: Create All Auxiliary Variables ####
  d4 = make_aux_vars(data=d3, aux.matrix=aux.matrix)
  
  ##### Step 4: Create All Missingness Indicators ####
  d5 = make_missing_indics(data=d4, miss.matrix=miss.matrix)
  
  ##### Step 5: Override Observations using Missingness Indicator ####
  d6 = missingify(d5, make.relateds.missing)
