...
time.name="t"
event.name="d"  # this is correct even for censored data; it's the indicator for whether an event occurred at that row
cluster.name="id"
cox.predictors = c("X")
...
impute.with = c("id", "X", "time.X", "time.Z", "Z", cox.predictors)
...
file.name = substring(data.path, 71)
