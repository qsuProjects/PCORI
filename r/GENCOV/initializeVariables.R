...
# use the program by Demirtas
source("/Volumes/QSU/Datasets/PCORI/data simulation/programs/jointly_generate_binary_normal.R")
...
n.Reps = 1  # number of simulations to run
i = rep(obs,n) # vector of IDs

varnames <- c("id", "male", 
              "d_abac", "d_ataz", "d_dida", "d_efav", "d_emtr", "d_indi", "d_lami", "d_lopi", "d_nelf", "d_nevi", "d_rito", "d_saqu", "d_stav", "d_teno", "d_zido",
              "age", "bmi", "cd4", "vln", "bps", "bpd", "ldl", "hdl", "trig",
              "ever_abac", "ever_ataz", "ever_dida", "ever_efav", "ever_emtr", "ever_indi", "ever_lami", "ever_lopi", 
              "ever_nelf", "ever_nevi", "ever_rito", "ever_saqu", "ever_stav", "ever_teno", "ever_zido")
...
# pmeans: means of non-drug normals, then mean proportion of time on drug
# pSDs: SDs of the above
# pvars: variance of the above
# pps: probability of ever being on each drug
...
pSDs <- c( 10.12,4.2,50,10000,15.48,9.96,23,7,30, sqrt( ( pmeans[c(10:24)]*( 1-pmeans[c(10:24)] ) ) / n ) )
pvars <- pSDs ^ 2
pps <- c(0.275, 0.222, 0.198, 0.50, 0.413, 0.206, 0.688, 0.253, 0.24, 0.184, 0.18, 0.103, 0.332, 0.526, 0.544, 0.97)


# population correlation matrix
pcorin <- read.csv("/Volumes/QSU/Datasets/PCORI/data simulation/generate covariates/Current simulation files/pcor.csv", header=FALSE)[-1,-1]
...
# within-subject variance for normal variables
wvar <- c(zero, zero, 1, 10, 10000, 5, 2, 3, 1)


# within-subject correlation matrix
wcorin <- read.csv("/Volumes/QSU/Datasets/PCORI/data simulation/generate covariates/Current simulation files/wcor_maya.csv", header=FALSE)[-1,-1]
